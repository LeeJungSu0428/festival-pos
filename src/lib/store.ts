...
type BaseGoods = {
  ...
  hasSizes: boolean;
  imageUrl: string | null;
  initialStockIfFlat: number;
  lowStockThresholdIfFlat: number;
};

// 10종 굿즈 원본 정보. 신소재/국제관 두 사이트가 이 10종을 각자 독립적인 재고로 취급한다
// (같은 이름의 상품이어도 두 사이트의 재고는 서로 전혀 연동되지 않는다).
const BASE_GOODS: BaseGoods[] = [
  {
    name: "야구 유니폼",
    price: 40000,
    cost: 31000,
    ...
    initialStockIfFlat: 0,
    lowStockThresholdIfFlat: 5,
  },
  {
    name: "축구 유니폼",
    price: 36000,
  ...
  },
  {
    ...
    initialStockIfFlat: 100,
    lowStockThresholdIfFlat: 20,
  },
  {
    name: "슬로건 카드",
    price: 7000,
  ...
  },
  {
    ...
    initialStockIfFlat: 50,
    lowStockThresholdIfFlat: 10,
  },
];

function buildDefaultProducts(): Product[] {
  const categories: ProductCategory[] = ["new-materials", "international-hall"];
  const products: Product[] = [];
  let n = 1;
  for (const category of categories) {
    for (const g of BASE_GOODS) {
      products.push({
        id: `p${n++}`,
        name: g.name,
        category,
        ...
        cost: g.cost,
        hasSizes: g.hasSizes,
        sizes: g.hasSizes ? sizeStockSet(20) : [],
        initialStock: g.hasSizes ? 0 : g.initialStockIfFlat,
        currentStock: g.hasSizes ? 0 : g.initialStockIfFlat,
        lowStockThreshold: g.lowStockThresholdIfFlat,
        imageUrl: g.imageUrl,
        active: true,
      });
    }
