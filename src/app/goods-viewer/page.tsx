    ...
    <div className="relative min-h-screen pb-32">
      ...
      <header className="flex items-center justify-between border-b border-[#D7E2EE] bg-[#FBFDFF] px-4 py-3 sm:px-6">
        <div>
          ...
          <h1 className="text-lg font-bold text-[#26415F]">굿즈 뷰어</h1>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            className={`rounded-full border px-3 py-1.5 text-xs font-medium ${
              editMode
                ? "border-[#3B5B82] bg-[#3B5B82] text-white"
                : "border-[#C9D6E4] text-[#26415F]"
            }`}
          >
      ...
      <main className="grid gap-4 p-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-3">
        {(state?.items ?? []).map((item) => (
          <div key={item.id} className="overflow-hidden rounded-xl border border-[#D7E2EE] bg-[#FBFDFF]">
            <div className="flex aspect-[20/9] items-center justify-center bg-[#E8EEF5]">
              {item.imageUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={item.imageUrl} alt={item.name} className="h-full w-full object-contain" />
              ) : (
                <div className="flex h-full items-center justify-center text-sm text-[#9AAEC4]">
                  이미지 없음
                </div>
              )}
            </div>
            ...
            <div className="p-4">
              {editMode ? (
                <div className="space-y-2">
                  <input
                    className="w-full rounded border border-[#C9D6E4] bg-white px-2 py-1.5 text-sm"
                    value={item.name}
                    onChange={(e) => saveItem(item, { name: e.target.value })}
                  />
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-[#5B6B82]">₩</span>
                    <input
                      type="number"
                      className="w-full rounded border border-[#C9D6E4] bg-white px-2 py-1.5 text-sm"
                      value={item.price}
                  ...
                  <input
                    className="w-full rounded border border-[#C9D6E4] bg-white px-2 py-1.5 text-xs"
                    value={item.imageUrl ?? ""}
                    onChange={(e) => saveItem(item, { imageUrl: e.target.value })}
                    placeholder="이미지 경로 또는 URL"
                  />
                  <label className="flex items-center gap-1.5 text-xs text-[#26415F]">
                    <input
                      type="checkbox"
                      checked={item.active}
                ...
                <>
                  <p className="font-semibold text-[#26415F]">{item.name}</p>
                  <p className="text-sm text-[#5B6B82]">₩{item.price.toLocaleString("ko-KR")}</p>
                </>
              )}
                      ...
                      <div key={size.label} className="flex items-center justify-between">
                        <button
                          ...
                          className={`text-sm font-medium ${
                            size.available ? "text-[#26415F]" : "text-[#9AAEC4] line-through"
